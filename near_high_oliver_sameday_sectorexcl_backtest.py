"""
=============================================================================
NEAR HIGH + OLIVER KELL -- SAME-DAY CLOSE ENTRY, SECTOR-EXCLUDED (Yahoo Finance)
=============================================================================
DATA      : Yahoo Finance (yfinance) — no login required. Daily candles,
            15 years of history by default (yfinance handles this fine --
            unlike hourly candles, daily data has no short Yahoo-side cap).
            NSE universe from NSE public CSV (~2500 EQ stocks).
            Batch downloads 50 stocks per request.

UNIVERSE  : Nifty 500 stocks (default) or All NSE EQ-series (~2500)
            Excludes: ETFs, Liquid Funds, Bonds, Index Funds
            PLUS excludes stocks whose sector (per the Nifty 500 sector
            map) is in EXCLUDED_SECTORS -- Realty, Diversified, IT, and
            Auto by default. These were the consistently weakest/most
            negative-PnL sectors across repeated backtest runs. Stocks
            outside the Nifty 500 sector map (niche/small-cap names with
            no known sector) are NOT excluded by this filter, since their
            fallback "sector" is just their own ticker, not a real
            classification. Adjust EXCLUDED_SECTORS to add/remove sectors.

PORTFOLIO : INITIAL_CAPITAL = Rs 1,00,000 (1 lakh)
            MAX_POSITIONS   = 4 (max 4 stocks held at any time)
            Compounding allocation: current total equity / 4 per position,
...
ENTRY TIMING (the difference vs near_high_oliver_combined.py):
            This variant assumes YOU are watching the market during the day
            and act before the close -- so a signal confirmed by today's
            price action is filled at TODAY's close, not next day's open.
            This is valid if you're actively monitoring intraday and place
            the order yourself near end-of-day; it is NOT valid for a fully
            unattended after-hours bot (use near_high_oliver_combined.py for
            that -- it delays entries to the next day's open specifically
            to avoid this same-day lookahead).
...
SIMULATION: Two-pass approach
            ...
            Pass 2 - Iterate day-by-day across all trading dates
              (a) Check all open positions for exits (using today's bar --
                  a position can only be checked for exit starting the day
                  AFTER it was entered, since entry already consumed today's
                  close)
              (b) Scan for new entry signals (if open < MAX_POSITIONS) and
                  enter IMMEDIATELY at today's close (with slippage, since
                  it's a market-style fill)
...
ENTRY     : EMA alignment (Close > EMA5 > EMA10 > EMA20 > EMA50 > EMA150 > EMA200)
          ...
          + Filled at TODAY's close (same bar the signal fired)
...
COSTS     : Slippage (0.05% default) applied to market-style fills only --
              same-day-close entries and Oliver_EMA_Stop/Extension_Exit/
...
TARGET_ABOVE_HIGH_PCT = 0.15                 # 15% above ref high
NEAR_HIGH_LOWER      = 0.90                  # 10% below high (lower bound)
NEAR_HIGH_UPPER      = 0.95                  # 5% below high (upper bound)
FETCH_YEARS          = 15                    # years of history to fetch
LOOKBACK_DAYS        = int(FETCH_YEARS * 365.25) - 400  # backtest window (leaves ~1yr warmup for EMA200/High_5Y)
...
CACHE_DIR            = "cache_near_high_sameday_15y"  # dedicated cache -- do NOT share with the
CACHE_FALLBACK_DIRS  = []                             # 6-year caches (cache_near_high_yf/cache_ath_yf);
                                                       # those only hold 6 years, which would silently
                                                       # truncate this script's 15-year window if reused.
...
# Sectors to exclude entirely from entries -- these were the consistently
# weakest/most negative-PnL sectors across repeated backtest runs of this
# strategy. A candidate whose sector_map lookup lands in this list is
# skipped regardless of signal quality. Only affects stocks the Nifty 500
# sector map actually classifies -- niche/small-cap stocks with no sector
# match are unaffected (see docstring).
EXCLUDED_SECTORS     = ["Realty", "Diversified", "IT", "Auto"]
...
def backtest_portfolio(
    ...
    # No "next day" stages here -- entries fill immediately at today's close.
    funnel = {
        "signals_raw": 0,                  # rows with Signal True on a scanned day (pre-filter)
        "lost_sector_excluded": 0,         # skipped: sector is in EXCLUDED_SECTORS
        "lost_sector_trend": 0,            # skipped: sector index not trending
        "lost_sector_diversity_scan": 0,   # skipped: sector slot already taken
        "lost_capital": 0,                 # not enough cash / <1 share at entry
    ...
    for day_i, dt in enumerate(all_dates, 1):
        ...
        # (A position entered today via STEP 2 has already consumed today's
        # close as its entry price, so it isn't eligible for an exit check
        # until tomorrow's bar -- bars_held naturally starts at 0 today.)
        # ================================================================
        symbols_to_close = []
        ...
        # ================================================================
        # STEP 2: SCAN FOR NEW ENTRY SIGNALS AND ENTER IMMEDIATELY AT
        # TODAY'S CLOSE (you're watching the market and act before the
        # close -- no next-day wait, unlike near_high_oliver_combined.py).
        # ================================================================
        ...
        if available_slots > 0:
            ...
            # Mark open positions to market for compounding position sizing
            ...
            # Enter IMMEDIATELY at today's close (with slippage) -- no queue.
            slots_filled = 0
            for cand in entry_candidates:
                ...
                # --- Sector exclusion (always applied, independent of USE_SECTOR_FILTER) ---
                if sec in EXCLUDED_SECTORS:
                    funnel["lost_sector_excluded"] += 1
                    continue
                ...
                # Market-style fill at today's close: adverse slippage applies
                ep = cand["close"] * (1 + SLIPPAGE_PCT / 100)
                if ep <= 0:
...
def performance_report(
    ...
    label: str = "NEAR HIGH + OLIVER KELL -- SAME-DAY CLOSE, SECTOR-EXCLUDED BACKTEST",
):
    sep = "=" * 70
    print(f"\n{sep}")
    print(f"  {label}")
...
def run_backtest(
    ...
    print("\n" + "=" * 70)
    print("  NEAR HIGH + OLIVER KELL -- SAME-DAY CLOSE, SECTOR-EXCLUDED BACKTEST")
    ...
    print(f"  Entry: SAME-DAY close (signal + fill on the same bar)  |  Slippage: {SLIPPAGE_PCT:.2f}% on market-style fills")
    ...
    if funnel:
        ...
        print(f"    Lost to excluded sector ({', '.join(EXCLUDED_SECTORS)})  : {funnel['lost_sector_excluded']:>7,}")
        print(f"    Lost to sector-trend filter                       : {funnel['lost_sector_trend']:>7,}")
        print(f"    Lost to sector-diversity cap                      : {funnel['lost_sector_diversity_scan']:>7,}")
        print(f"    Lost: insufficient capital / <1 share at entry    : {funnel['lost_capital']:>7,}")
        print(f"    Filled (became a trade, at today's close)         : {funnel['filled']:>7,}\n")
    ...
    performance_report(
        trades_df,
        equity_curve_df,
        label="NEAR HIGH + OLIVER KELL -- SAME-DAY CLOSE, SECTOR-EXCLUDED BACKTEST",
    )

    # Save results
    ts  = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_trades = f"near_high_oliver_sameday_sectorexcl_trades_{ts}.csv"
    trades_df.to_csv(out_trades, index=False)
    print(f"  Trades saved to: {out_trades}")

    out_equity = f"near_high_oliver_sameday_sectorexcl_equity_{ts}.csv"
