"""
=============================================================================
NEAR HIGH + OLIVER KELL -- NEXT-DAY OPEN ENTRY, SECTOR-EXCLUDED (Yahoo Finance)
=============================================================================
...
CACHE_DIR            = "cache_near_high_sameday_15y"  # shared with the sameday scripts -- the raw
CACHE_FALLBACK_DIRS  = []                             # OHLCV/indicator data is identical regardless of
                                                       # entry timing, so reusing this dir lets you skip
                                                       # re-fetching 15 years of data if you've already
                                                       # run near_high_oliver_sameday_backtest.py or
                                                       # near_high_oliver_sameday_sectorexcl_backtest.py.
...
def performance_report(
    ...
    label: str = "NEAR HIGH + OLIVER KELL -- NEXT-DAY OPEN, SECTOR-EXCLUDED BACKTEST",
):
    sep = "=" * 70
    print(f"\n{sep}")
    print(f"  {label}")
...
def run_backtest(
    ...
    print("\n" + "=" * 70)
    print("  NEAR HIGH + OLIVER KELL -- NEXT-DAY OPEN, SECTOR-EXCLUDED BACKTEST")
    ...
    performance_report(
        trades_df,
        equity_curve_df,
        label="NEAR HIGH + OLIVER KELL -- NEXT-DAY OPEN, SECTOR-EXCLUDED BACKTEST",
    )

    # Save results
    ts  = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_trades = f"near_high_oliver_nextday_sectorexcl_trades_{ts}.csv"
    trades_df.to_csv(out_trades, index=False)
    print(f"  Trades saved to: {out_trades}")

    out_equity = f"near_high_oliver_nextday_sectorexcl_equity_{ts}.csv"
